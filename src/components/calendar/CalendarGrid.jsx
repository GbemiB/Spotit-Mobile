    ...
    <View>
      ...
      <View style={s.grid}>
        {cells.map((d, i) => {
          ...
          const ph     = phaseKey ? { color: phases[phaseKey] } : phaseFor(cycleDayOf(iso, lastPeriodDate, cycleLength), periodLength, cycleLength);
          ...
          return (
            <Pressable key={i} onPress={() => onSelect(iso)} style={s.cell}>
              <View style={[
                ...
                <Text style={[s.dayText, isToday && { color: colors.white, fontWeight: '600' }, !isToday && !ph.color && { color: colors.textMuted }]}>
                  {d}
                </Text>
