...
describe('HYDRATE', () => {
  ...
  test('forces a logout when authDone is true but there is no access token', () => {
    // Regression: corrupted/partial persisted state (authDone true, tokens null) used to
    // render the main app with every authenticated call permanently 401ing, and no way back
    // to login — the 401-retry path is itself gated on having a token to refresh.
    const corrupted = {
      onboarded: true,
      authDone: true,
      accessToken: null,
...
describe('SAVE_LOG', () => {
  ...
  test('a newly-earned entry updates balance, streak, and history', () => {
    ...
    expect(next.longestStreak).toBe(5); // unchanged, 3 doesn't beat the existing record
...
describe('profile and notifications', () => {
  test('PROFILE_HYDRATED assigns fields as-is, including explicit nulls', () => {
    // GET/PATCH /users/me return the full current profile, not a partial patch — a
    // genuinely-cleared lastPeriodDate must overwrite stale local state, not be ignored.
